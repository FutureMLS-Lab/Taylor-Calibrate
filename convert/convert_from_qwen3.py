...
# Convert pretrained Qwen3 HF weights to FLA-compatible TransformerForCausalLM format.
...
config_dict = {
    # NOTE: 0.6B has head_dim=128 but hidden_size/num_heads=64. FLA TransformerConfig
    # computes head_dim=64 (hidden_size//num_heads), causing Q/K shape mismatch on copy.
    # Qwen3-0.6B conversion via this script is NOT supported. Use Qwen3-8B (head_dim matches).
    "4b": {
...
def convert(
    ...
    for i in range(config_obj.num_hidden_layers):
        ...
        if hasattr(model.model.layers[i].attn, 'q_norm'):
            ...
            target_dtype = model.model.layers[i].attn.q_norm.weight.dtype
            model.model.layers[i].attn.q_norm.weight.data.copy_(llama.model.layers[i].self_attn.q_norm.weight.to(target_dtype))
            torch.testing.assert_close(model.model.layers[i].attn.q_norm.weight, llama.model.layers[i].self_attn.q_norm.weight.to(target_dtype))
        if hasattr(model.model.layers[i].attn, 'k_norm'):
            ...
            target_dtype = model.model.layers[i].attn.k_norm.weight.dtype
            model.model.layers[i].attn.k_norm.weight.data.copy_(llama.model.layers[i].self_attn.k_norm.weight.to(target_dtype))
            torch.testing.assert_close(model.model.layers[i].attn.k_norm.weight, llama.model.layers[i].self_attn.k_norm.weight.to(target_dtype))
